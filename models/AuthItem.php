...
class AuthItem extends \yii\db\ActiveRecord
{
    ...
    public static function getRoles()
    {
        return static::find()
            ->select(['role_name'])
            ->where(['>', (new Expression('length(role_name)')), 0])
            ->andWhere(['name' => 'head_of_department'])
            ->orWhere(['name' => 'head_teacher'])
            ->orWhere(['name' => 'teacher'])
            ->indexBy('name')
            ->column();
    }

    public static function getRolesForAdding()
    {
        return static::find()
            ->select(['role_name'])
            ->where(['>', (new Expression('length(role_name)')), 0])
            ...
            ->orWhere(['name' => 'parent_of_student'])
            ->indexBy('name')
            ->column();
    }
}
